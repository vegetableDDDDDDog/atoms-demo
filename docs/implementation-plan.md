...
**Architecture:** 继续使用当前 Next.js + React 前端原型。所有能力先在本地确定性实现，不接后端、不接数据库、不接真实 AI。核心原则是 query 驱动生成结果，不按固定业务模板穷举。
...
**目标：** 先用 localStorage 保存完整 session，不急着引入数据库。
...
- [ ] 说明本地运行方式。
- [ ] 说明当前不接后端、不接数据库、不接真实 AI 的原因：先验证核心交互闭环。
